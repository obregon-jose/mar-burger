...
export function Promotions() {
  ...
  // Obtener productos disponibles con promociones activas
  const availableProductsWithPromotions = data.products
    .filter((product) => {
      // Solo productos habilitados y disponibles
      return product.enabled && product.stock === "available"
    })
    .map((product) => {
      ...
      const promotion = data.promotions.find((promo) => {
        ...
        // Verificar si la promoción está activa (entre fechas y habilitada)
    ...
    .filter((item) => item.hasPromotion) // Solo mostrar productos con promociones activas

  const handleOrderClick = (productName: string) => {
    const message = `¡Hola! Me interesa la promoción de ${productName} 🍔`
  ...
  const prevSlide = () => {
    setCurrentSlide(
      (prev) => (prev - 1 + availableProductsWithPromotions.length) % availableProductsWithPromotions.length,
    )
  }
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 sm:mb-16">
            ...
            {/* <p className="text-gray-300 text-base sm:text-lg max-w-2xl mx-auto">
              Próximamente tendremos increíbles ofertas para ti
            </p> */}
          </div>
          <div className="text-center">
            <div className="bg-gray-900 border border-red-600/20 rounded-2xl p-8 sm:p-12 max-w-md mx-auto">
              {/* <Percent className="w-12 h-12 sm:w-16 sm:h-16 text-red-500 mx-auto mb-4" /> */}
